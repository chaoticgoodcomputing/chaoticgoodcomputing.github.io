...
/**
 * Handles pan and zoom interactions for mermaid diagram popups
 */
...
/** CSS variables to extract from document for mermaid theming */
const _cssVars = [
  "--secondary",
  "--tertiary",
  "--gray",
...
/**
 * Extract CSS variable values from document root
 */
function _getCSSVariables(): Record<(typeof _cssVars)[number], string> {
  return _cssVars.reduce(
    (acc, key) => {
      acc[key] = window.getComputedStyle(document.documentElement).getPropertyValue(key)
      return acc
    },
    {} as Record<(typeof _cssVars)[number], string>,
  )
}

/**
 * Position the expand button relative to the clipboard button
 */
function _positionExpandButton(
  pre: HTMLPreElement,
  clipboardBtn: HTMLButtonElement,
...
/**
 * Create a show handler for the mermaid diagram popup
 */
function _createShowHandler(
  codeBlock: HTMLElement,
  popupContainer: HTMLElement,
): { show: () => void; hide: () => void; panZoom: DiagramPanZoom | null } {
...
/**
 * Initialize mermaid diagrams and set up interactive controls
 */
async function setupMermaidDiagrams() {
  ...
  // Store original text for re-rendering
  ...
  /**
   * Render all mermaid diagrams with current theme
   */
  async function renderMermaid() {
    // Reset any previously processed diagrams
    for (const node of nodes) {
  ...
  // Set up expand functionality for each diagram
  for (let i = 0; i < nodes.length; i++) {
    ...
    _positionExpandButton(pre, clipboardBtn, expandBtn)

    const popupContainer = pre.querySelector("#mermaid-container") as HTMLElement
    if (!popupContainer) continue

    const { show, hide } = _createShowHandler(codeBlock, popupContainer)

    expandBtn.addEventListener("click", show)
    registerEscapeHandler(popupContainer, hide)

    window.addCleanup(() => {
      expandBtn.removeEventListener("click", show)
    })
  }
}

document.addEventListener("nav", setupMermaidDiagrams)
